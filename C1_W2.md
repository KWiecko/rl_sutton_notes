...
$$\sum_{s'\in S}\sum_{r\in R}p(s',r|s,a) = 1, \forall s\in S, r\in R$$
...
$$p(s'|s, a) = Pr [{S_{t} =s'|S_{t-1}=s, A_{t-1}=a}]  = \sum_{r\in R}p(s',r|s,a)$$

Expected reward for state-action pairs (two-argument function)

$$r(s,a) = E[R_{t} = r| S_{t-1} = s, A_{t-1} = a] = \sum_{r \in R} r \sum_{s' \in S} p(s',r|s,a)$$

Expected reward for stat-action-next state triplets (three argument function)

$$r(s, a, s') = E[R_{t}|S_{t} = s', S_{t-1} = s, A_{t-1} = a] = \sum_{r \in R} r \frac{p(s', r|s, a)}{p(s'|s,a)}$$
...
$$G_{t} = R_{t+1} + R_{t+2} + R_{t+3} + R_{t+4}+...R_{T}$$
...
$$G_{t} = R_{t+1} + \gamma R_{t+2} + \gamma^2 R_{t+3} + \gamma^3 R_{t+4}+...= \sum_{k}^{\infin} \gamma^{k} R_{t+k+1}$$
...
$$G_{t} = \sum_{k=0}^{\infin} \gamma^{k} = \frac{1}{1-\gamma}$$
